...
public class StateType implements RuntimeType {
	...
	public StateInstance newInstance() throws ConnectionError, RuntimeError {
	...
	public void build(StateInstance uin) throws ConnectionError, RuntimeError {
 		if (hasBuilds) {
	...
	public void initialize(StateInstance uin, StateRunnable parent, boolean includeDerivedVariables) throws RuntimeError {
        ...
        if (includeDerivedVariables) {
            ...
            for (PathDerivedVariable pdv : pathderiveds) {
            	if (!varHM.containsKey(pdv.varname)) {
            		throw new RuntimeError("No such pd variable " + pdv.varname + " in variables map: " + varHM);
            	}
            ...
            for (ExpressionDerivedVariable edv : exderiveds) {
            	if (!varHM.containsKey(edv.varname)) {
            		throw new RuntimeError("No such ed variable " + edv.varname + " in variables map: " + varHM);
            	}
	...
	public void applyPathDerived(StateInstance uin) throws RuntimeError {
		for (PathDerivedVariable pdv : pathderiveds) {
			if (pdv.isSimple()) {
					...
					if (si != null) {
						...
						if (pdv.isRequired()) {
							throw new RuntimeError("Required path variable is missing: " + pdv);
						}
					}
	...
	public void evaluate(StateInstance uin, StateRunnable parent) throws RuntimeError {
		HashMap<String, DoublePointer> varHM = uin.getVarHM();
	...
	private void evalDerived(StateInstance uin, HashMap<String, DoublePointer> varHM, StateRunnable parent) throws RuntimeError, RuntimeError {
		if (indeps != null) {
			for (String s : indeps) {
	...
	public void eulerAdvance(StateInstance uin, StateRunnable parent, double t, double dt) throws RuntimeError, RuntimeError {
 		HashMap<String, DoublePointer> varHM = uin.getVarHM();
		varHM.get("t").set(t);
	...
	public void rk4Advance(StateInstance uin, StateRunnable parent, double t, double dt) throws RuntimeError, RuntimeError {
	...
	void evalDerivs(HashMap<String, Double> v0, double t, HashMap<String, Double> ret) throws RuntimeError {
    ...
    private void synchronizeExposures(StateInstance uin) throws RuntimeError {
 		HashMap<String, DoublePointer> varHM = uin.getVarHM();
		...
		for (String s : exposedMap.keySet()) {
			...
			if (expHM.get(s) == null) {
				throw new RuntimeError("No such exposure: " + s);
				...
				// throw new RuntimeError("No such variable " + st);
			}
...
*/
	...
	public StateType getConsolidatedStateType(String knownas) throws RuntimeError {
	...
	public StateType getFlattenedStateType(String knownas) throws RuntimeError {
		if (flattenedCB == null) {
	...
	public StateType makeConsolidatedStateType(String knownas) throws RuntimeError {
		StateType ret = null;
		if (simultaneous) {
	...
	public StateType makeChildConsolidated() throws RuntimeError {
	...
	private void consolidateChildren() throws RuntimeError {
	...
	public StateType makeFlattened(String knownas) throws RuntimeError {	
	...
	private void addToFlattener(String pfx, Flattener fl) throws RuntimeError {
		String fullpfx = "";
	...
	public void addStateVariable(String s, String dim) throws RuntimeError {
		svars.add(s);
		if (dim == null) {
			throw new RuntimeError("Null dimension for " + s);
		}
		dimensions.put(s, dim);
	}
	
	public void addPathDerivedVariable(PathDerivedVariable pdv) throws RuntimeError {
		pathderiveds.add(pdv);
		String d = pdv.getDimensionString();
		if (d == null) {
			throw new RuntimeError("Null dimension for " + pdv);
		}
		dimensions.put(pdv.getVariableName(), d);
	}
	
	public void addExpressionDerivedVariable(ExpressionDerivedVariable edv) throws RuntimeError {
		exderiveds.add(edv);
		String d = edv.getDimensionString();
		if (d == null) {
			throw new RuntimeError("Null dimension for " + edv);
		}
		dimensions.put(edv.getVariableName(), d);
	}
	
	public void addVariableROC(VariableROC vroc) throws RuntimeError {
		rates.add(vroc);
		String d = vroc.getDimensionString();
		if (d == null) {
			throw new RuntimeError("Null dimension for " + vroc);
		}
	...
	public String getPropertyStringValue(String sp) throws RuntimeError {
		String ret = null;
		if (refHM.containsKey(sp)) {
			...
			throw new RuntimeError("Can't get property " + sp + " on " + this);
		}
		return ret;
	}
	...
	public StateType makeShallowCopy() throws RuntimeError {
		StateType ret = new StateType(cptid, typeName);
	...
	public String getDimensionString(String fld) throws RuntimeError {
		String ret = null;
		if (dimensions.containsKey(fld)) {
			...
			ret = "(none)";
			// throw new RuntimeError("No dimension for " + fld + " in " + this);
		}
		return ret;
	}
