...
import org.lemsml.jld.eval.Plus;
...
public class Flattener {
	...
	public void addIndependentVariable(String s, String d) throws RuntimeError {
		if (indepsA.contains(s)) {
			...
			dimensions.put(s, d);
			if (d == null) {
				throw new RuntimeError("Null dimension for independent variable " + s);
			}
		}
	}
	
	
	public void add(PathDerivedVariable pdv) throws RuntimeError {
 		pdvA.add(pdv);
 		String d = pdv.getDimensionString();
 		dimensions.put(pdv.getVariableName(), d);
 		if (d == null) {
			throw new RuntimeError("Null dimension for path derived variable " + pdv);
		}
 //		E.info("added pdv " + pdv);
	}

	public void add(ExpressionDerivedVariable edv) throws RuntimeError {
		edvA.add(edv);
		String d = edv.getDimensionString();
		dimensions.put(edv.getVariableName(), d);
		if (d == null) {
			throw new RuntimeError("Null dimension for expression derived variable " + edv);
		}
		//		E.info("added edv " + edv);
	}

	public void add(VariableROC vroc) throws RuntimeError {
		rocA.add(vroc);	
		String d = vroc.getDimensionString();
		dimensions.put(vroc.getVariableName(), d);
		if (d == null) {
			throw new RuntimeError("Null dimension for rate variable " + vroc);
		}
	}

	public void addStateVariable(String sv, String dim) throws RuntimeError {
		svA.add(sv);
		if (dim == null) {
			throw new RuntimeError("Null dimension for stae variable " + sv);
		}
...
//				E.info("match " + sa + " * " + sb + " from " + vnm + " leaves " + inner);
	...
	public void exportTo(StateType ret) throws RuntimeError {
