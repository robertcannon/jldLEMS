...
public class LemsLiteFactoryGenerator {

	 
	

	private String generateJava() {
		LemsLiteClasses lclasses = LemsLiteClasses.getInstance();
		...
		sb.append("public class LemsLiteFactory extends AbstractLemsLiteFactory {\n\n\n");
		...
		for (Field f : c.getFields()) {
			if (java.lang.reflect.Modifier.isPublic(f.getModifiers())) {
				...
				if (exfn != null) {
					...
					if (fnm.startsWith("x_")) {
						fnm = fnm.substring(2, fnm.length());
						sb.append("            } else if (xn.equals(\"" + fnm + "\")) {\n");
						sb.append("                ret." + f.getName() + " = " + exfn + "(xv);\n");
	...
	public static void main(String[] argv) throws IOException {
		LemsLiteFactoryGenerator lfg = new LemsLiteFactoryGenerator();
		
		String txt = lfg.generateJava();
 		
		File f = new File("src/org/lemsml/jlems/core/reader/LemsLiteFactory.java");
